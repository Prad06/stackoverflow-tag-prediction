...
def logisticRegressionOut(
    ...
    if model_save_path:
        joblib.dump({"model": model, "label_encoder": label_encoder}, model_save_path)
...
def randomForestOut(
    ...
    for tree_size in tree_sizes:
        ...
        print(
            f"Trees: {tree_size}, Test Error: {test_error:.4f}, Train Error: {np.mean(kf_errors):.4f}, F1 Score: {f1:.4f}, AUC Score: {auc_score:.4f}"
        )
